...
`chapter_anchor` may be `null` (densified/10%-fallback checkpoints). `snippet_anchor` key is required but MAY be an empty string — a textless-prefix checkpoint (e.g. image-only front matter) legitimately has no snippet and the device falls back to chapter/percent anchors (still spoiler-safe). Checkpoint `percent` values must be strictly ascending — so `plan_checkpoints` MUST coalesce any consecutive checkpoints that round to the same integer percent (a short interior chapter can otherwise produce two checkpoints at the same percent, which fails `validate()` and crashes generation AFTER the full API budget is spent).
...
- [ ] **Step 3: Implement** `xray_core/schema.py`: `SCHEMA_VERSION = 1`; `validate()` checks: required top-level keys and types, `schema_version == 1`, fingerprint keys, every checkpoint has `percent` (int 1–100, strictly ascending, last == `last_percent`), a `snippet_anchor` key present (string, MAY be empty), `snapshot` with the four lists, characters/locations entries have `name`, `first_pct`, `first_seq`, and `first_pct <= checkpoint.percent` (D4 guard). Write `schema/xray.schema.json` mirroring it.
...
**Coalesce equal-percent checkpoints (required — else generation crashes at `validate()`):** integer `percent` can repeat when two boundaries are <1% of the book apart (a short interior chapter). After building `cps`, collapse any run of consecutive checkpoints with the same `percent` into one — keep the LAST (largest offset) of the run so coverage stays gapless, and never drop the forced-100 final checkpoint. `schema.validate()` requires strictly-ascending percent, so an uncoalesced duplicate makes `generate_xray` raise *after* spending the whole API budget. Add `test_no_duplicate_percents_short_interior_chapter` (a book with one <1%-length interior chapter → `plan_checkpoints` → `validate()` of a doc built from it is clean).
...
`ui.py`: `InterfaceAction` named `"X-Ray Generator"`; `genesis()` sets menu action → `generate_selected()`: for each selected book id with an EPUB format, **on the GUI thread copy the EPUB into the per-book workdir via `db.new_api.copy_format_to(book_id, "EPUB", dest)`** (NOT `format_abspath` — its docstring warns it "breaks the threadsafe promise"; handing a raw library path to a worker races a concurrent library op) → dispatch a `ThreadedJob` running `read_epub` → `generate_xray(..., calibre_uuid=db.new_api.field_for("uuid", book_id), progress_cb=job.set_progress, workdir=<plugin tmp>)` → `embed_xray` to a temp file. **calibre calling-convention gotchas (verified against live calibre source):** (a) `ThreadedJob` calls the worker `func(*args, **kwargs)` and INJECTS `log`/`abort`/`notifications` into kwargs — so the worker function MUST take its business args positionally FIRST and declare `log`/`abort`/`notifications` as keyword-only (`def _work(epub, uuid, ..., *, log, abort, notifications)`), or they collide with positional binding and raise `TypeError`. (b) `ThreadedJob`'s completion `callback` runs on the WORKER thread — it MUST be wrapped in `calibre.gui2.Dispatcher(self._job_done)` (as calibre's own first-party actions do) before it touches any Qt widget/dialog or `library_view.model().refresh_ids(...)`; driving Qt off the GUI thread is a thread-affinity violation. **Before replacing the library file, validate the temp EPUB** — `zipfile.ZipFile(tmp).testzip() is None` AND `read_embedded(tmp)` round-trips the doc AND `read_epub(tmp)` still parses; only then, on the GUI thread, `db.add_format(book_id, "EPUB", tmp_path, replace=True)`. If validation fails, abort that book and leave the library file untouched (an `embed_xray` bug must never become permanent data loss). Errors → `error_dialog`; partial (`complete=False`) → warning dialog "prepared up to X% — run again to resume". Books without EPUB format are skipped with a summary dialog.
